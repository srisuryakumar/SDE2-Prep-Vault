...
- Problem Statement: Find the top 3 highest paid employees in each department.
- Solution:
  ```postgresql
  ...
  WHERE rank <= 3;
  ```
- Note: Window functions like DENSE_RANK() and ROW_NUMBER() are the standard way to solve "top N per category" problems in interviews.
